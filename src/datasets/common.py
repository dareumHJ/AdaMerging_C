'''Common utilities for dataset handling in AdaMerging_C project.'''
...
## return ImageFolder related dictionary with images' datapaths
## In other words, override ImageFolder to return image paths along with images and labels
...
## Helper function to convert batch to dictionary format
...
## Helper function to extract feature vectors using image encoder(Image processing part of CLIP model)
def get_features_helper(image_encoder, dataloader, device):
    ...
    ### change model to evaluation mode(No dropout, no batchnorm update, etc.)
    image_encoder.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            for key, val in batch.items():
                if key == 'images':
                    continue
                if hasattr(val, 'cpu'): ### in case val is a tensor
                    val = val.cpu()
                    all_data[key].append(val)
                else:                   ### in case val is a list
...
## Main function to get features from dataset using image encoder (Use cache for speed up if available)
def get_features(is_train, image_encoder, dataset, device):
    ...
    if image_encoder.cache_dir is not None:
        ...
        # if exists, load features from there
    ...
    if image_encoder.cache_dir is not None and len(cached_files) > 0:
        ...
        for cached_file in cached_files:
            ### make a key(name) for data dictionary
...
## Inherit Dataset class to create a dataset from extracted features
## Why this class is needed? -> Use pre-extracted features to train a linear classifier
## Speed up! No need to pass images through image encoder every time (SKIP the image encoder part)
class FeatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ### create a data dictionary for the given index
        data = {k: v[idx] for k, v in self.data.items()}
        ### convert features to torch tensor
        data['feautures'] = torch.from_numpy(data['features']).float()
        return data
    

## Determine whether to use feature dataset or original dataset
def get_dataloader(dataset, is_train, args, image_encoder=None):
    ...
    if image_encoder is not None:
        ...
        ### shuffle only if is_train is True
...
## Newly Implemented for AdaMerging (shuffle test dataloader)
