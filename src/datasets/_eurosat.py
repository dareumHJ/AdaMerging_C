...
import re # 정규표현식을 위한 모듈? 이라는데...
"""eurosat이 좀 복잡한 이유? -> CLIP 모델이 이미지를 더 잘 알아듣도록,
                  이미지 클래스 이름을 좀 더 자연스럽게 바꿔주기 위해서"""
...
def pretify_classname(classname):
    ...
    ### 공백으로 단어 연결
    out = ' '.join(l)
    ### industrial, residential 같은 단어 뒤에 area 추가
    if out.endswith('al'):
        return out + ' area'
    return out
